...
std::set<uint_fast8_t> getPossibleValues(boardType &board, vector<vector<bool>> fixedOrMarked, int row, int col);
...
//    }
    ...
    while (hasChange) {
        ...
        for (int i = 0; i < board.N; i++) {
            ...
            if(res.first != -1 && res.second != -1){
                ...
                fixedOrMarked[i][res.first] = true;
                goto restart;
            }
        }
        ...
        for (int i = 0; i < board.N; i++) {
            ...
            if(res.first != -1 && res.second != -1){
                ...
                fixedOrMarked[res.first][i] = true;
                goto restart;
            }
    ...
    //fill in missing values randomly
...
//            fixedOrMarked[lastEncounter] = true;
//            return true;
...
std::set<uint_fast8_t> getPossibleValues(boardType &board, vector<vector<bool>> fixedOrMarked, int row, int col) {
    ...
    for (int i = 0; i < board.N; i++) {
        if (fixedOrMarked[row][i])
            encountered[board.board[row][i]] = true;
        if (fixedOrMarked[i][col])
